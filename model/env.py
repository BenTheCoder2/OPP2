...
class PackingEnv():
    ...
    def step(self):
        ...
        # Training
        # store experience
        self.replay = [self.state, (self.seq, self.place), self.reward, self.state_]
        self.replay_buffer.append(self.replay)
        ...
        self.batch = random.sample(self.replay_buffer, self.batch_size)


        # preprocess batch
        states, actions, rewards, next_states = zip(*self.batch)

        actions_seq, actions_place = zip(*actions)

        states = tf.convert_to_tensor(states, dtype=tf.int32)
        actions_seq = tf.convert_to_tensor(actions_seq, dtype=tf.int32)
        actions_place = tf.convert_to_tensor(actions_place, dtype=tf.int32)
        ...
        if tf.reduce_max(actions_place).numpy() >= 37:
            print("Invalid index found in actions_place:", actions_place.numpy())
            raise ValueError("Action index out of range for q_place shape")

        # Compute gradients
        with tf.GradientTape() as tape:
            q_seq = self.policy_net_seq(states)

            q_seq = tf.gather(q_seq, tf.cast(actions_seq, tf.int32), axis=1, batch_dims=1)  

            next_q_values_seq = self.target_net_seq(states)
            max_next_q_values_seq = tf.reduce_max(next_q_values_seq, axis=1)
            target_q_values_seq = rewards + discount_rate* max_next_q_values_seq

            tape.watch(self.policy_net_seq.trainable_variables)
            loss_seq = tf.keras.losses.MSE(target_q_values_seq, q_seq)
        seq_gradients = tape.gradient(loss_seq, self.policy_net_seq.trainable_variables)
        
        with tf.GradientTape() as tape:
            q_place = self.policy_net_place(states, 37)

            q_place = tf.gather(q_place, tf.cast(actions_place, tf.int32), axis=1, batch_dims=1)  

            next_q_values_place = self.target_net_place(states)
            max_next_q_values_place = tf.reduce_max(next_q_values_place, axis=1)
            target_q_values_place = rewards + discount_rate* max_next_q_values_place

            tape.watch(self.policy_net_place.trainable_variables)
            loss_place = tf.keras.losses.MSE(target_q_values_place, q_place)
        place_gradients = tape.gradient(loss_place, self.policy_net_place.trainable_variables)


        # Apply gradients
        self.optimizer_seq.apply_gradients(zip(seq_gradients, self.policy_net_seq.trainable_variables))

        self.optimizer_place.apply_gradients(zip(place_gradients, self.policy_net_place.trainable_variables))

        # update target network
        if episode % target_update_freq == 0:
            self.target_net_seq.set_weights(self.policy_net_seq.get_weights())

        if episode % target_update_freq == 0:
            self.target_net_place.set_weights(self.policy_net_place.get_weights())
